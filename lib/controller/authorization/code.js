...
module.exports = function(req, res, client, scope, user, redirectUri, pref) {
    ...
    async.waterfall([
        ...
        function(cb) {
            if (pref) {
                req.oauth2.logger.debug('Client was already authorized');
                cb();
            } else if (!req.body || typeof(req.body['decision']) == 'undefined')
        ...
        // save user auth pref
        function(cb) {
            req.oauth2.model.user.saveClientAuthorizationPref(user, client, function(err) {
                if (err)
                    cb(new error.serverError('Error while saving user authorization preference'));
                else
                    cb();
            })
        },
