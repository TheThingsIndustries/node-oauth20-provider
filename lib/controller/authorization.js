...
module.exports = function(req, res, next) {
    ...
    async.waterfall([
        ...
        // fetch user client authorization preference for client
        function(cb) {
            req.oauth2.model.user.clientAuthorizationPref(user, client, function (err, pref) {
                if (err)
                  cb(new error.serverError('Error in fetching authorization preference'));
                else {
                  authPref = pref;
                  cb();
                }
    ...
    function(err) {
        if (err) response.error(req, res, err, redirectUri);
            ...
            if (req.method == 'GET' && !authPref)
                req.oauth2.decision(req, res, client, scope, user, redirectUri);
            else if (grantType == 'authorization_code')
                authorization.code(req, res, client, scope, user, redirectUri, authPref);
